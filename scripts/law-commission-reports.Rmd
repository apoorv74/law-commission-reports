...
lc_links_title <- dplyr::left_join(lc_links, all_reports, by=NULL)
lc_links_title <- lc_links_title %>% group_by(`Report ID`) %>% mutate(group_id=row_number())
lc_links_title <- lc_links_title[lc_links_title$group_id == 1,]
all_reports$`Report ID` <- as.character(all_reports$`Report ID`)

view_reports <- left_join(all_reports, lc_links_title, by=NULL)
